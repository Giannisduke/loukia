<div class="wrap">
    ...
    <form name="dofollow" action="options.php" method="post">
        ...
        <select style="width:98%;" rows="1" cols="57" id="coinimp_currentcurrency" name="coinimp_currentcurrency" onchange='setSiteKey()'>
            <?php
            for ($x = 0; $x <= (count(get_option('coinimp_currencies')) - 1); $x++) {
                echo "<option " . ((get_option('coinimp_currentcurrency') == $x) ? "selected " : "") . "value='$x'>". strtoupper(get_option('coinimp_currencies')[$x]) . "</option>";
            }
            echo "</select>";
            ?>
        <h3 class="coinimp-labels" for="coinimp_sitekey">
